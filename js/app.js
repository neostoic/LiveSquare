...
/*	Old Method of adding venues
			// Read the initial list of data from Firebase and add it to the list and map
			fire.once('value', function(venueSnapshot) {


				// Loop through each venue and add it to the list
				venueSnapshot.forEach(function(venueChildSnapshot) {	
...
/*	
	// Add venue to list and map when it is added to Firebase
	firebaseRef.on('child_added', function(snapshot, prevChildName) {
				
		// Place the venue data into an object and compile an Underscore template
		var venue = venueChildSnapshot.val();
		console.log("Adding venue to app: " + venue.name)
		console.log("Venue object is:");
		console.log(venue);
		// Pass the venue through the render function
		var template = render(venue);
		...
		// Incriment the venue number
		v++;

	});
	
	firebaseRef.on('child_moved', function(childSnapshot, prevChildName) {
